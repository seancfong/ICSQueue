...
const Header = (props: Props) => {
  ...
  return (
    <div className="sticky font-primary w-full flex justify-center items-center py-3 top-0 z-50 bg-gradient-to-l from-[#0064a4] to-[#1b3d6d] shadow-lg">
      ...
      {/* Admin */}
      {isAdmin && (
        <div className="absolute left-0 top-0 h-full flex flex-col justify-center items-center pl-8 md:pl-16 text-white">
          <Link href="/admin">
            <span className="hidden md:block">Manage</span>
            <BsShieldFillCheck className="block md:hidden w-5 h-5 p-[2px]" />
          </Link>
        </div>
      )}
      ...
      {/* Username */}
      {user?.displayName && (
        <div className="absolute right-0 top-0 h-full flex flex-col justify-center items-center pr-8 md:pr-16 text-white">
          <button
            ...
            <AnimatePresence>
              {showLogout && (
                <motion.div
                  ...
                  className="absolute top-2 w-20 md:w-32 h-12 bg-slate-100 bg-opacity-90 rounded-lg text-gray-700 flex justify-center items-center shadow-md"
                >
